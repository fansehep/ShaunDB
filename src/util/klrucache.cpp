...
bool kLRUCache::get(const std::string& key, std::string* value, bool* isErase) {
  ...
  if (cache_iter->second->isHot == false) {
    LOG_TRACE("get in cold_data");
    // protected_list_.push_front(*(cache_iter->second));
    // cache_map_[key] = protected_list_.begin();
    // protected_list_.begin()->isHot = true;
    ...
    // push 到冷数据队列中
    // 让冷数据队列 pop 出去
    LOG_INFO("here? mem_size: {} max_size: {}", protected_memory_usage_,
             max_protected_mem_size_);
    while (protected_memory_usage_ >= max_protected_mem_size_) {
      auto rese_iter = protected_list_.rbegin();
      const auto rese_kv_size = (rese_iter->key->size()) + rese_iter->value.size();
      LOG_INFO("rese_kv_size: {}", rese_kv_size);
      // 冷数据的一条记录
...
void kLRUCache::set(const std::string& key, const std::string& value) {
  ...
  while (probation_memory_usage_ >= max_probation_mem_size_) {
    ...
    // map 先删除
    rese_iter->key = &(cache_map_.find(key)->first);
    auto rese_kv_size = (rese_iter->key->size() + rese_iter->value.size());
    cache_map_.erase(*(rese_iter->key));
    probation_list_.pop_back();
    probation_memory_usage_ -= rese_kv_size;
    LOG_INFO(
        "erase cold data cur_probation_memory_size: {} max_probation_mem_size: "
        "{}",
        probation_memory_usage_, max_probation_mem_size_);
  }
}
